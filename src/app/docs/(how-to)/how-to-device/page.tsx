...
export default function HowToDevice() {
  ...
  const Row = ({
    ...
    <div className="flex items-center justify-between py-2.5 border-b border-border last:border-0">
      ...
      <span
        className={`text-xs font-semibold ${value === true ? "text-success-600" : value === false ? "text-error-500" : "text-foreground"}`}
      >
        {String(value)}
      </span>
    </div>
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto px-6 pt-14 pb-24">
        ...
        <p className="text-sm text-muted mb-10">
          Live readout of your current device, browser, and environment.
        </p>
        ...
        <div className="space-y-8">
          <section>
            ...
            <div className="p-6 rounded-2xl border border-border bg-surface">
              ...
              <Row
                label="device.hasTouchScreen()"
                value={device.hasTouchScreen()}
              />
              <Row
                label="device.getViewport()"
          ...
          <section>
            ...
            <div className="p-6 rounded-2xl border border-border bg-surface space-y-3">
              <div
                className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${online ? "border-success-200 bg-success-50" : "border-error-200 bg-error-50"}`}
              >
                <span
                  className={`w-2.5 h-2.5 rounded-full ${online ? "bg-success-500" : "bg-error-500"} animate-pulse`}
                />
                <span
                  className={`text-sm font-semibold ${online ? "text-success-700" : "text-error-600"}`}
                >
                  {online ? "Online" : "Offline"}
                </span>
              </div>
              <p className="text-xs text-muted">
                Try disconnecting your network to see this update in real time.
              </p>
            </div>
            <p className="mt-2 text-xs text-muted font-mono">
              device.onConnectivityChange(callback)
            </p>
          </section>
          ...
          <section>
            ...
            <div className="p-6 rounded-2xl border border-border bg-surface">
              ...
              <Row
                label="device.prefersReducedMotion()"
                value={device.prefersReducedMotion()}
              />
          ...
          <section>
            <p className="text-xs font-bold uppercase tracking-widest text-error-500 mb-3">
              4 — Browser
            </p>
            <div className="p-6 rounded-2xl border border-border bg-surface">
              <Row label="browser.getName()" value={browser.getName()} />
              <Row
                label="browser.supportsWebP()"
                value={browser.supportsWebP()}
              />
              <Row
                label="browser.supportsWebSocket()"
                value={browser.supportsWebSocket()}
              />
              <Row
                label="browser.supportsServiceWorker()"
                value={browser.supportsServiceWorker()}
              />
            </div>
            <p className="mt-2 text-xs text-muted font-mono">
